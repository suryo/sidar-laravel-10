...
<div class="fixed inset-0 z-50 bg-black flex flex-col">
    ...
    <div class="absolute top-0 left-0 right-0 p-4 flex justify-between items-center bg-gradient-to-b from-black/70 to-transparent z-10 text-white">
        <div class="flex items-center">
            ...
            <span class="text-sm font-medium uppercase">Absence Masuk</span>
        </div>
    ...
    <div class="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black/80 to-transparent z-10 flex flex-col items-center">
        ...
        <form id="attendance-form" action="{{ route('attendance.check-in') }}" method="POST" enctype="multipart/form-data" class="hidden fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full max-w-xs bg-white/10 backdrop-blur-md p-6 rounded-xl border border-white/20 shadow-2xl z-50">
            @csrf
            ...
            <div class="mb-3 text-center">
                <label class="block text-xs font-medium text-white mb-1">Tipe Kerja</label>
                <select name="work_type" class="block w-full text-sm rounded bg-white/80 border-0 p-2 text-gray-900 text-center">
                    <option value="wfo">Work From Office</option>
                    <option value="wfh">Work From Home</option>
                    <option value="outside">Dinas Luar</option>
                </select>
            </div>
            ...
            <div class="flex space-x-2">
                ...
                <button type="submit" class="flex-1 py-2 px-4 bg-primary-600 hover:bg-primary-500 text-white rounded text-sm font-medium">
                    Submit Absen
                </button>
            </div>
        </form>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        saveMessageBtn.addEventListener('click', () => {
            ...
            // Optional: Show some feedback like changing icon color
        ...
        navigator.mediaDevices.getUserMedia({ video: { facingMode: 'user' } })
            ...
            .catch(err => {
                ...
                loading.innerHTML = '<span class="text-red-500">Camera Access Denied. Please enable permissions.</span>';
            });
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(
                (position) => {
                    ...
                    locationText.innerText = `Lat: ${position.coords.latitude.toFixed(4)}, Lng: ${position.coords.longitude.toFixed(4)}`;
                    // Optionally reverse geocode here
                },
                (error) => {
                    console.error("Location Error:", error);
                    locationText.innerText = "Location Denied";
                    locationText.classList.add('text-red-400');
                }
            );
        }
        ...
        captureBtn.addEventListener('click', () => {
            ...
            // Set input value (Base64 is native here, controller checks for file object so we might need to adjust or handle Base64)
            // Note: The previous form used input type="file". We need to handle base64 string in controller OR convert to blob.
            // For simplicity, let's inject into a hidden input and assume controller handles "base64" string or file.
            
            // Actually, standard Laravel `request->file()` won't work with base64 string directly unless we handle it.
            // We'll rename the input to `photo_base64` to differentiate, or converting blob is better but harder in vanilla JS without fetch logic.
            // Let's stick to base64 string in a hidden text input, and I will update controller to handle it.
            document.getElementById('photo-data').value = dataUrl;
            document.getElementById('photo-data').name = "photo_base64"; 

            // Show Form & Hide Capture Button
