...
<div class="max-w-lg mx-auto bg-white shadow overflow-hidden sm:rounded-lg">
    ...
    <form action="{{ route('master.departments.store') }}" method="POST" class="px-4 py-5 sm:p-6 space-y-4">
        @csrf
        <div>
            <label for="code" class="block text-sm font-medium text-gray-700">Code</label>
            <input type="text" name="code" id="code" required class="mt-1 shadow-sm focus:ring-primary-500 focus:border-primary-500 block w-full sm:text-sm border-gray-300 rounded-md">
        </div>
        <div>
